    ...
    if end[stock].iloc[-1] > 20 and volumn[stock].iloc[-1] > 1000000:
        ...
        if end[stock].iloc[-1] < start[stock].iloc[-1]:
            ...
            if last_10_days.sum() < 6:
                last_60_days_low = low[stock].iloc[-60:].min()
                if end[stock].iloc[-1] <= last_60_days_low*1.05:
