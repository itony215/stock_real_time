...
etf = pd.read_csv("/home/pineapple/Documents/stock/crawler/ETF_buy/隔日衝占比_20221121.csv")
today = date.today()
...
get_list = []
yesterday = '2022-11-21'
for stock_id in tqdm(three.index.levels[0]):
    try:
        df0 = end_price_handle(stock_id,str(yesterday))
        df1 = three_handle(stock_id,str(yesterday))
        df2 = main_handle(stock_id,str(yesterday))
        df3 = earn_handle(stock_id)
        df4 = stock_list.loc[stock_id]
        df4['id'] = str(df4.name)
        df5 = start_price_handle(stock_id,str(yesterday))
        df6 = high_price_handle(stock_id,str(yesterday))
        df7 = volumn_handle(stock_id,str(yesterday))
        df10 = margin_handle(stock_id,str(yesterday))
        ...
        break
#print()
result = pd.DataFrame(get_list,columns=['id','name','category','昨日收盤價','昨日漲跌幅','開盤位置','開盤價','收盤價','漲跌幅','A轉跌幅','成交量','三大買賣超','融券5N','昨日融券','融券差額(張)','融券增加','投信買賣超(張)','投信買賣超%','投信持股比例',\
