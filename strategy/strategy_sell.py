...
for stock_id in tqdm(three.index.levels[0]):
    try:
        ...
        break
#print()
result = pd.DataFrame(get_list,columns=['id','name','category','昨日收盤價','昨日漲跌幅','開盤位置','開盤價','收盤價','漲跌幅','A轉跌幅','成交量','三大買賣超','融券5N','昨日融券','融券差額(張)','融券增加','融資5N','昨日融資','融資差額(張)','投信買賣超(張)','投信買賣超%','投信持股比例',\
