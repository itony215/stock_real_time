    ...
    for entry in it:
        if entry.name.endswith(".csv") and entry.is_file():
            ...
            if(df.iloc[-1]['count']>2 and df.iloc[-1]['漲跌幅']>1 and df.iloc[-1]['收盤價']>20 and df.iloc[-1]['量']>1000):
                ...
                notice += stock_id+stock_list[stock_list.index==stock_id]['證券名稱'].values[0]+': '+str(df.iloc[-1]['收盤價'])+'\n'
                print(stock_id,stock_list[stock_list.index==stock_id]['證券名稱'].values[0],df.iloc[-1]['收盤價'])
